...
node* constructFromPrePost(vector<int>& pre, vector<int>& post ,int &preIndex,int  &posIndex) {
        node* root = newNode(pre[preIndex++]);
        if (root->data != post[posIndex])
            root->left = constructFromPrePost(pre, post , preIndex , posIndex);
        if (root->data != post[posIndex])
            root->right = constructFromPrePost(pre, post , preIndex , posIndex);
        posIndex++;
        return root;
    }
int main()
